...
def compute_average_acuracy(sess, prediction):
    ...
    for epoch in range(int(0.25*epoch_max)):
      # Get data
      n = get_rule()
...
def predict_class(sess, n, k, r, prediction):
    ...
    for pred in range(int(0.10*epoch_max)):
...
if __name__ == '__main__':
  ...
  with tf.Session() as sess:
    ...
    if there_is_checkpoint:
      ...
      if predict_average >= 0.9:
        print("Saving Neural Network...")
        save_path = saver.save(sess, checkpoint_prefix)


    if predict_average < 0.9:
      print("Acuracy is less than necessary value! Training again!")
    else:
